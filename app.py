...
# --- Główna funkcja aplikacji, chroniona hasłem ---
def main_app():
    # --- Konfiguracja ---
    ...
    # --- Logika aplikacji (przeniesiona do funkcji) ---
    st.title("📝 Notes Git")
...
# --- GŁÓWNA LOGIKA STRONY SPRAWDZAJĄCA HASŁO ---
st.set_page_config(page_title="Notes Git", page_icon="📝")

# Sprawdź, czy hasło jest ustawione w sekretach
if "APP_PASSWORD" not in st.secrets:
    st.error("Błąd krytyczny: Hasło aplikacji nie zostało ustawione w 'Secrets'.")
    st.stop()

# Poproś użytkownika o hasło
password = st.text_input("Podaj hasło, aby uzyskać dostęp", type="password")

# Sprawdź hasło
if password == st.secrets.APP_PASSWORD:
    # Jeśli hasło się zgadza, uruchom właściwą aplikację
    main_app()
elif password:
    # Jeśli podano hasło, ale jest niepoprawne
