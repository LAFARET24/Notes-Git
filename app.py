...
def run_main_app():
    ...
    if "file_id" not in st.session_state:
        with st.spinner("Sprawdzanie archiwum..."):
            ...
            with st.spinner("Przetwarzam..."):
                if is_saving:
                    ...
                    if not notes_content.strip():
                        response_text = "Twoje archiwum jest jeszcze puste."
                    else:
                        system_prompt = (f"Jesteś asystentem, który odpowiada na pytania wyłącznie na podstawie dostarczonych notatek z pamiętnika.\n"
                                         f"Oto notatki:\n{notes_content}\n\nPYTANIE: {prompt_text}")
                        response = model.generate_content(system_prompt)
                        response_text = response.text
                st.markdown(response_text)
                sound_file = text_to_audio(response_text)
                if sound_file: st.audio(sound_file, autoplay=True)
...
if "APP_PASSWORD" not in st.secrets:
    st.error("Błąd: Hasło aplikacji nie jest ustawione w 'Secrets'.")
    st.stop()
